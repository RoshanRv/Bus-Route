...
// add the bus stop details by user
app.post("/stopdata", (req, res) => {
  console.log("heehe");
  const { latitude, longitude, cityName, stopName } = req.body;
  try {
    db.query(
      "select * from stopbus where cityName=? and stopName=?",
      [cityName, stopName],
      (err, row) => {
        if (err) console.log(err);
        console.log(row);
        if (row.length > 0) {
          let updatedcount = row[0].count + 1;
          db.query(
            "update stopbus set count=? where id=?",
            [updatedcount, row[0].id],
            (err, rows) => {
              if (err) console.log(err);
          ...
          db.query(
            "insert into stopbus(latitude,longitude,cityName,stopName,count) values(?,?,?,?,?)",
            [latitude, longitude, cityName, stopName, 1],
            (err, rows) => {
              if (err) console.log(err);
...
app.post("/getstopdata", (req, res) => {
  const { cityName } = req.body;
  console.log(cityName);
  try {
    db.query(
      "select * from stopbus where cityName=? AND count>10",
      [cityName],
      (err, row) => {
        console.log(row);
